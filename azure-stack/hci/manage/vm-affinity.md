...
A Windows PowerShell használatával egyszerűen hozhat létre affinitási és affinitási szabályokat a fürtben lévő virtuális gépekhez (VM).

Az affinitás egy olyan szabály, amely kapcsolatot létesít két vagy több erőforráscsoport vagy szerepkör, például virtuális gépek (VM-EK) között, hogy azok együtt maradjanak egy Azure Stack HCI-fürtben. Az affinitás az ellentétes, hogy a megadott erőforráscsoportok egymástól eltérőek maradnak.
...
Az affinitás és az affinitási szabályok használata esetén a fürtözött virtuális gépek bármelyike ugyanabban a fürtcsomóponton marad, vagy nem lehet együtt ugyanabban a fürtcsomóponton.  Így a virtuális gépek egy csomóponton kívülre való áthelyezésének egyetlen módja a manuális elvégzése.  A virtuális gépeket a VHDX található Fürt megosztott kötete (CSV) együtt is megtarthatja.

Az affinitás és az affinitási szabályok kombinálásával egy kibővített fürtöt is beállíthat két hely között (tartalék tartományok), és megtarthatja a virtuális gépeket a helyen.

A szabályokat általában egy távoli számítógépről kell beállítania, nem pedig a fürtben lévő gazdagépen. Ezt a távoli számítógépet felügyeleti számítógépnek nevezzük.

> [!NOTE]
> Ha felügyeleti számítógépről futtat PowerShell-parancsokat, adja meg a-Name vagy a-cluster paramétert a kezelt fürt nevével. Ha szükséges, meg kell adnia a teljes tartománynevet (FQDN) is, ha a-számítógépnév paramétert használja egy kiszolgáló-csomóponthoz.

## <a name="new-powershell-cmdlets"></a>Új PowerShell-parancsmagok

Azure Stack HCI-fürtökhöz tartozó affinitási szabályok létrehozásához használja a következő PowerShell-parancsmagokat.

### <a name="new-clusteraffinityrule"></a>Új – ClusterAffinityRule
...
### <a name="move-clustergroup"></a>Áthelyezés – Clustergroup felsorolásban találhatók

Az új `-IgnoreAffinityRule` kapcsoló figyelmen kívül hagyja a szabályt, és áthelyezi a fürtözött erőforráscsoportot egy másik fürtcsomópont-csomópontra. További információ erről a parancsmagról: [Move-clustergroup felsorolásban találhatók](/powershell/module/failoverclusters/move-clustergroup?view=win10-ps).
...
### <a name="start-clustergroup"></a>Start – Clustergroup felsorolásban találhatók

Az új `-IgnoreAffinityRule` kapcsoló figyelmen kívül hagyja a szabályt, és az aktuális helyen online állapotba hozza a fürtözött erőforráscsoportot. További információ erről a parancsmagról: [Start-clustergroup felsorolásban találhatók](/powershell/module/failoverclusters/start-clustergroup?view=win10-ps).
...
## <a name="anti-affinity-rules"></a>Affinitási szabályok
...
## <a name="affinity-rules"></a>Affinitási szabályok

Íme néhány gyakori forgatókönyv az affinitási szabályok beállításához.
...
## <a name="combined-rules"></a>Egyesített szabályok
