---
title: Virtuális gépek kezelése a Windows PowerShell-Azure Stack HCI használatával
...
A virtuális gépeket általában egy távoli számítógépről kell kezelni, nem pedig a fürtben lévő gazdagépen. Ezt a távoli számítógépet felügyeleti számítógépnek nevezzük.
...
A virtuális gépek PowerShell használatával történő kezelésével kapcsolatos teljes dokumentációért lásd: [Hyper-V referenciák](/powershell/module/hyper-v/?view=win10-ps).

## <a name="create-a-vm"></a>Virtuális gép létrehozása  

A `New-VM` parancsmag egy új virtuális gép létrehozásához használatos. A részletes használatért tekintse meg a [New-VM](/powershell/module/hyper-v/new-vm?view=win10-ps) Reference dokumentációját.
...
- **– A kapcsoló** annak a virtuális kapcsolónak a neve, amelyet a virtuális gépnek más virtuális gépekhez vagy hálózathoz való kapcsolódáshoz használni kíván. Szerezze be a virtuális kapcsoló nevét a [Get-VMSwitch](/powershell/module/hyper-v/get-vmswitch?view=win10-ps)használatával. Például:  
...
Ha olyan virtuális gépet szeretne létrehozni, amely az operációs rendszer lemezképéhez tartozó új virtuális lemezzel rendelkezik, tekintse meg a [Windows 10 rendszerű Hyper-V virtuális gép létrehozása](/virtualization/hyper-v-on-windows/quick-start/create-virtual-machine)bemutatójának PowerShell-példáját.  
...
Az alábbi példa a kiszolgálón futó virtuális gépek listáját adja vissza egy szűrő hozzáadásával a `Where-Object` paranccsal. További információ: [a WHERE-Object dokumentáció használata](/previous-versions/windows/it-pro/windows-powershell-1.0/ee177028(v=technet.10)) .
...
A `Start-VM` és a `Stop-VM` parancsok használatával indítsa el vagy állítsa le a virtuális gépet. Részletes információkat a [Start-VM](/powershell/module/hyper-v/start-vm?view=win10-ps) és a [stop-VM](/powershell/module/hyper-v/stop-vm?view=win10-ps) dokumentációjában talál.
...
A `Move-VM` parancsmag egy virtuális gépet egy másik kiszolgálóra helyez át. További információt a [Move-VM](/powershell/module/hyper-v/move-vm?view=win10-ps) dokumentációjában talál.
...
A `Import-VM` és a `Export-VM` parancsmag egy virtuális gép importálását és exportálását. Az alábbiakban néhány példát mutatunk be. További információ: [import-VM](/powershell/module/hyper-v/import-vm?view=win10-ps)  és [export-VM](/powershell/module/hyper-v/export-vm?view=win10-ps) Reference dokumentáció.
...
A `Rename-VM` parancsmag használatával lehet átnevezni a virtuális gépeket. Részletes információkért tekintse meg az [Átnevezés – virtuális gép](/powershell/module/hyper-v/rename-vm?view=win10-ps) referenciájának dokumentációját.
...
A `Checkpoint-VM` parancsmag egy virtuális gép ellenőrzőpontjának létrehozására szolgál. Részletes információkért tekintse meg az [ellenőrzőpont – virtuális gép](/powershell/module/hyper-v/checkpoint-vm?view=win10-ps) dokumentációját.
...
A `New-VHD` parancsmag használatával új virtuális merevlemez hozható létre egy virtuális géphez. További információ a használatáról: [új VHD-](/powershell/module/hyper-v/new-vhd?view=win10-ps) dokumentáció.
...
A `Add-VMNetworkAdapter` parancsmag használatával egy virtuális hálózati adaptert adhat hozzá egy virtuális géphez. Az alábbiakban néhány példát mutatunk be. A használatáról részletes információt az [Add-VMNetworkAdapter](/powershell/module/hyper-v/add-vmnetworkadapter?view=win10-ps) dokumentációjában talál.
...
A `New-VMSwitch` parancsmag a virtuálisgép-gazdagépek új virtuális kapcsolójának használatára szolgál. A használatával kapcsolatos részletes információkat a [New-VMSwitch](/powershell/module/hyper-v/new-vmswitch?view=win10-ps) Reference dokumentációban talál.
...
A `Set-VMMemory` parancsmag a virtuális gép memóriájának konfigurálására szolgál. A használatáról részletes információt a [set-VMMemory](/powershell/module/hyper-v/set-vmmemory?view=win10-ps) dokumentációjában talál.
...
A `Set-VMProcessor` parancsmag használatával konfigurálhatja a virtuális gép virtuális processzorait. A használatáról részletes információt a [set-VMProcessor](/powershell/module/hyper-v/set-vmprocessor?view=win10-ps) dokumentációjában talál.
...
A virtuális gépeket a Windows felügyeleti központban is létrehozhatja és kezelheti. További információ: [Windows felügyeleti központ](vm.md).
