...
Ez a cikk segítséget nyújt a Azure Stack hub alapszintű platformokra vonatkozó követelményeinek ellenőrzéséhez a NVA-konfigurációk esetében.
...
2. NVA teljesítményének keresése.
...
1. Annak ellenőrzését, hogy a NVA a következő ugrás-e, futtassa a `Tracert <Private IP of NVA>` Windows vagy a rendszert `Traceroute <Private IP of NVA>` .
...
1. Győződjön meg arról, hogy bármely hálózati biztonsági csoport (NSG) blokkolja-e a forgalmat. Windows, Run `ping` (ICMP) vagy `Test-NetConnection <Private IP of NVA>` (TCP) esetén. Linux esetén futtassa a parancsot `Tcpping <Private IP of NVA>` .
...
A vagy a futtatása közben [`PsPing`](/sysinternals/downloads/psping) `Nmap` rögzítse egyidejű hálózati nyomkövetést a forrás-és a cél virtuális gépeken, valamint a NVA. Ezután állítsa le a nyomkövetést.
...
2. Használja `PsPing` vagy a `Nmap` forrás virtuális gépről a cél virtuális gépre. Ilyenek például a következők: `PsPing 10.0.0.4:80` vagy `Nmap -p 80 10.0.0.4` .

3. Nyissa meg a hálózati nyomkövetést a célként megadott virtuális gépen a **tcpdump** vagy a kívánt csomagkapcsolt analizátor használatával. Alkalmazzon egy megjelenítési szűrőt a futtatott vagy a forrásként szolgáló virtuális gép IP-címére `PsPing` `Nmap` . Egy Windows **netmon** például: `IPv4.address==10.0.0.4` . A Linux-példák a `tcpdump -nn -r vmtrace.cap src` és a `dst host 10.0.0.4` .
