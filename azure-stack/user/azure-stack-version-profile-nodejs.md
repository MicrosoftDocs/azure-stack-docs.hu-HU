...
Egy API-profil az erőforrás-szolgáltatók és API-verziók. Az API-profilok segítségével egy erőforrás-szolgáltatói csomag az egyes erőforrástípusok legújabb és legnagyobb stabil verziójának beszerzéséhez.
  ...
  -   Az összes szolgáltatás legújabb verzióját használni, hogy a **legújabb** profil olyan csomagot.

  -   A Azure stack-kompatibilis szolgáltatások használatához használja az  **\@Azure/ARM-Resources-Profile-Hybrid-2019-03-01** vagy  **\@az Azure/ARM-Storage-Profile-2019-03 -01-Hybrid**
...
A szolgáltatás legújabb API-verziójának használatához használja az adott ügyféloldali függvénytár **legújabb** profilját. Ha például az erőforrás-szolgáltatás legújabb API-verzióját szeretné használni, használja `azure-arm-resource` az erőforrás- **kezelési ügyfél függvénytárának** profilját. csomag.
...
3. Telepítse vagy frissítse a [Visual Studio Code](https://code.visualstudio.com/) -ot, és telepítse a [Node. js](https://code.visualstudio.com/docs/Node.js/nodejs-debugging) -bővítményt a Visual Studio Code-hoz.
...
3.  A csomagokat, amelyeket telepíteni kell a használni kívánt profilt verziójától függ. Az erőforrás-szolgáltatók listáját a [csomagok a NPM](#packages-in-npm) szakaszban találja.
...
7.  Győződjön meg arról, hogy az egyszerű szolgáltatás közreműködői/tulajdonosi szerepkörrel rendelkezik az előfizetésében. Szerepkör hozzárendelése egyszerű szolgáltatást, lásd: [alkalmazások elérést biztosíthat az Azure Stack](../operator/azure-stack-create-service-principals.md).

### <a name="nodejs-prerequisites"></a>A Node. js előfeltételei 

Ha a Node. js Azure SDK-t Azure Stack használatával szeretné használni, meg kell adnia a következő értékeket, majd értékeket kell beállítania környezeti változókkal. A környezeti változók beállítása, tekintse meg az alábbi táblázat az operációs rendszerének utasításokat.

| Érték | Környezeti változók | Leírás |
| --- | --- | --- |
| Bérlőazonosító | BÉRLŐ\_AZONOSÍTÓJA | A Azure Stack [bérlő azonosítójának](https://docs.microsoft.com/azure/azure-stack/azure-stack-identity-overview)értéke. |
...
| Előfizetés azonosítója | Az\_előfizetés\_- [azonosító](https://docs.microsoft.com/azure/azure-stack/azure-stack-plan-offer-quota-overview#subscriptions) az Azure-előfizetés azonosítója, hogy hogyan érheti el az ajánlatokat a Azure Stackban.  |
...
| Resource Manager-végpont | ARM\_-VÉGPONT | Lásd [a Azure stack Resource Manager](https://docs.microsoft.com/azure/azure-stack/user/azure-stack-version-profiles-ruby#the-azure-stack-resource-manager-endpoint)-végpontot. |
...
-  **\@Azure/ARM-resourceprovider-Profile-2019-03 -01-Hybrid**
    ...
    A Azure Stackhez készült legújabb profil. Ez a profil Services kompatibilis az Azure Stack mindaddig, amíg a 1808 blokk vagy a további használhatják.
...
Az Azure Stacket és API-profilokkal kapcsolatos további információkért lásd: egy [összegzése az API-profilok](https://docs.microsoft.com/azure/azure-stack/user/azure-stack-version-profiles#summary-of-api-profiles).

### <a name="azure-nodejs-sdk-api-profile-usage"></a>Azure Node. js SDK API-profil használata

A profil-ügyfél létrehozásához a következő sorokat kell használni. Ez a paraméter csak akkor szükséges, az Azure Stack vagy egyéb privát felhők. A globális Azure-ban a (vagy @azure-arm-resource @azure-arm-storage) alapértelmezés szerint már rendelkezik ezekkel a beállításokkal.
...
## <a name="environment-settings"></a>Környezet beállításai

Az egyszerű szolgáltatásnév Azure Stack-környezetbe való hitelesítéséhez használja a következő kódot: A kód használata és a környezeti változók beállítása a parancssorban automatikusan létrehozza ezt a leképezést a fejlesztő számára.
...
2.  Azure-beli szolgáltatásnév létrehozása, és rendelje hozzá egy szerepkörhöz az előfizetés eléréséhez. Útmutatásért lásd: a [Azure PowerShell használata egy egyszerű szolgáltatásnév létrehozásához tanúsítvánnyal](https://docs.microsoft.com/azure/azure-stack/azure-stack-create-service-principals).
...
5.  Nyissa meg a minta alkalmazás fájlját.`index.js`
